...
import {
    Col, Row, Card, CardBody, CardTitle, Button, Form, FormGroup, Label, Input, CardSubtitle,
    FormText,
} from 'reactstrap';
import Web3 from "web3";
import MedisecureABI from "../../../MedisecureABI.json"; // Path to your contract ABI
...
const AddMedicine = ({ user, history }) => {
    ...
    defaultFormData.manufacturer = user.name;
    ...
    const createMedicine = async () => {
        ...
        if ( !formData.medicine_id || !formData.description || !formData.antibiotic || !formData.category || !formData.formula || !formData.name || !formData.chemicals ) {
            Swal.fire({
        ...
        const data = { ...formData };
        const chemicals = data.chemicals ? data.chemicals.split(",").map(e => e.trim()) : [];
        data.chemicals = chemicals.join(",");

        try {
            await contract.methods.add_medicine(
                data.medicine_id,
                data.name,
                data.description,
                user.name,
                        ...
                        <Form onSubmit={submitForm}>
                            <Row form>
                                <Col md={6}>
                                    <FormGroup>
                                        ...
                                        <Input value={medicine_id} onChange={(e) => onInput(e, "medicine_id")} type="text" name="medicine_id" id="medicine_id"
                            ...
                            <Row>
                                <Col md={12}>
                                    <FormGroup>
                                        ...
                                        <Input value={description} onChange={(e) => onInput(e, "description")} type="textarea" name="description" id="description" placeholder="Enter Description"/>
                                    </FormGroup>
                                </Col>
                            </Row>
                            <Row form>
                                <Col md={6}>
                                    <FormGroup>
                                        ...
                                        <Input value={category} onChange={(e) => onInput(e, "category")} type="text" name="category" id="category"
                                ...
                                <Col md={6}>
                                    <FormGroup>
                                        ...
                                        <Input value={formula} onChange={(e) => onInput(e, "formula")} type="text" name="formula" id="formula"
                            ...
                            <FormGroup tag="fieldset" row>
                                ...
                                <Col md={6}>
                                    <FormGroup check>
                                        <Label check>
                                            <Input checked={antibiotic === "antibiotic"} onChange={(e) => antibioticToggle("antibiotic")} type="radio" name="medicineType" />{' '}
                                            Antibiotic
                                        </Label>
                                    </FormGroup>
                                    <FormGroup check>
                                        <Label check>
                                            <Input checked={antibiotic === "non-antibiotic"} onChange={(e) => antibioticToggle("non-antibiotic")} type="radio" name="medicineType" />{' '}
                                            Non-Antibiotic
                                        </Label>
                                    </FormGroup>
                                </Col>
                            ...
                            <Row>
                                <Col md={12}>
                                    <FormGroup>
                                        ...
                                        <Input value={chemicals} onChange={(e) => onInput(e, "chemicals")} placeholder="Enter all chemicals used in production of this medicine" type="textarea" name="chemicals" id="chemicals" />
