...
router.get('/getOrderById/:id', (req, res) => {
    ...
    const order = CustomerOrders.findOne({ _id: id })
        .populate("products.distributorStock customer", "-password -token");
    order.then((allCustomerOrders) => {
        res.send({ result: allCustomerOrders })
    }).catch(e => {
        res.send({ message: e.message });
...
router.get('/getOrdersByDistributor/:id', (req, res) => {
    var id = req.params.id
    const stock = CustomerOrders.find({ distributor: id })
        .populate("products.distributorStock customer");
    stock.then((allCustomerOrders) => {
        // console.log(allCustomerOrders, 'custtt')
...
router.get('/getOrdersByCustomerId/:id', (req, res) => {
    var id = req.params.id
    const stock = CustomerOrders.find({ customer: id })
        .populate("distributorStock");
    stock.then((allCustomerOrders) => {
...
router.put('/update', async (req, res) => {
    try {
        ...
        if (data._id) {
            ...
            await CustomerOrders.updateOne({ _id: data?._id }, { status: data?.status });
